...
function Scroll({
  ...
  const childrenCallbackRef = useCallback<
  ...
  const addPullIndicator = (el: HTMLDivElement) => {
    ...
    pullIndicator.className = [
      ...
      classNames?.pullIndicator,
    ].join(' ')
    pullIndicator.innerHTML = pullIndicatorComponent
      ? renderToString(pullIndicatorComponent)
      : ''
  ...
  const handleTouchStart = (startEvent: TouchEvent) => {
    ...
    const handleTouchEnd = (endEvent: TouchEvent) => {
      ...
      const indicator = scrollRef.current?.querySelector(
        `.${styles['pull-indicator']}`,
      )
  ...
  useEffect(() => {
    ...
    const existingLoader = contentRef.current.querySelector(
